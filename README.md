...
5. Leg B runs the second-call/merge verification while Leg A plays the pure in-band DTMF-9 tone loop (852 Hz + 1336 Hz — no spoken “guard test” audio).
...
- `public/` — static assets incl. `verify-tone.wav` (pure DTMF-9 merge tone) and the exact relayguard challenge-noise WAV/MP3 probes
